...
export default {
  'simple test': async api => {
    ...
    assertResult(response, RESPONSE_FIXTURES.header, 'getLedger')
  },
  'by hash': async api => {
    const response = await api.getLedger({
      ledgerHash:
    ...
    assertResult(response, RESPONSE_FIXTURES.headerByHash, 'getLedger')
  },
  ...
  'with state as hashes': async api => {
    ...
    assertResult(response, RESPONSE_FIXTURES.withStateAsHashes, 'getLedger')
  },
  'with settings transaction': async api => {
    ...
    const response = await api.getLedger(request)
    assertResult(response, RESPONSE_FIXTURES.withSettingsTx, 'getLedger')
  },
  'with partial payment': async api => {
    ...
    const response = await api.getLedger(request)
    assertResult(response, RESPONSE_FIXTURES.withPartial, 'getLedger')
  },
  'pre 2014 with partial payment': async api => {
    ...
    const response = await api.getLedger(request)
    assertResult(response, RESPONSE_FIXTURES.pre2014withPartial, 'getLedger')
  },
  'full, then computeLedgerHash': async api => {
    ...
    const response = await api.getLedger(request)
    assertResult(response, RESPONSE_FIXTURES.full, 'getLedger')
    const ledger = {
      ...response,
