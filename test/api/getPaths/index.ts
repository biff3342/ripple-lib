...
import { assertResult, assertRejects, TestSuite } from '../utils'
...
export default {
  'simple test': async api => {
    ...
    assertResult(response, RESPONSE_FIXTURES.XrpToUsd, 'getPaths')
  },
  'queuing': async api => {
    ...
    assertResult(normalResult, RESPONSE_FIXTURES.XrpToUsd, 'getPaths')
    assertResult(usdOnlyResult, RESPONSE_FIXTURES.UsdToUsd, 'getPaths')
    assertResult(xrpOnlyResult, RESPONSE_FIXTURES.XrpToXrp, 'getPaths')
  },
  // @TODO
  ...
  'getPaths USD 2 USD': async api => {
    ...
    assertResult(response, RESPONSE_FIXTURES.UsdToUsd, 'getPaths')
  },
  'getPaths XRP 2 XRP': async api => {
    const response = await api.getPaths(REQUEST_FIXTURES.XrpToXrp)
    assertResult(response, RESPONSE_FIXTURES.XrpToXrp, 'getPaths')
  },
  ...
  'send all': async api => {
    ...
    assertResult(response, RESPONSE_FIXTURES.sendAll, 'getPaths')
  },
} as TestSuite
